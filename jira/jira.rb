...
Puppet::Reports.register_report(:jira) do
  ...
  def process
   begin
     ...
     req = Net::HTTP::Get.new("#{uri.path}/api/hosts/#{self.host}")
     req.add_field('Accept', 'application/json,version=2' )
     req.content_type = 'application/json'
     req.basic_auth "admin", "123456"
     response = http.request(req)
     response_json = JSON.parse(response.body)
     issue_id = response_json["comment"]
     changeIssueStatus issue_id
   rescue Exception => e
     raise Puppet::Error, "Could not send report to Foreman at #{foreman_url}/api/reports: #{e}\n#{e.backtrace}"
   end
  end

  def changeIssueStatus issue_id
    ...
    path = '/rest/api/2/issue/' + issue_id + '/transitions?expand=transitions.fields'
